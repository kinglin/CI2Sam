...
class OutputUtil:
    ...
    def output_indv_list(filename, indv_tuple_list, final_population):
        ...
        df_result = pd.DataFrame(columns=list(indv_tuple_list[0][4].columns.values))
        ...
        for i in range(len(indv_tuple_list)):
            ...
            df_result.append(indv_tuple_list[i][4])

        for i in range(len(final_population)):
            data = {'number': [i],
                    'ma_method': [final_population[i].ma_method],
                    'comb': [final_population[i].l_s_values],
                    'extent': [final_population[i].fuzzy_extent],
                    'rating_value': [final_population[i].rating_value]}
            data_df = pd.DataFrame(data)
